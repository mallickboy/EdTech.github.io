...
function emailVerification(app,user,res) {
    console.log("email Verification");
    console.log(user.newUserEmail);
    const htmlContent = `
    <!DOCTYPE html><html><head><title>Login</title><style>body {
        background-color: antiquewhite
        }</style></head><body><h2></h2><div class="loginContainer"><div class="loginHeading"><h1>Almost Done</h1><span class="hoverPopout" id="closeForm" onclick="displayLogin()">X</span></div><form class="myForm" action="/courses" method="post"><input class="myInput" type="email" name="verifyEmail" value="${user.newUserEmail}" placeholder="${user.newUserEmail}" readonly><input class="myInput" type="number" name="clientOtp" placeholder="Enter your 6 digit ReLearn OTP here"><button class="btn hoverPopout" name="entry" value="3">Verify </button></form><style>@import url('https://fonts.googleapis.com/css2?family=Bree+Serif&family=Indie+Flower&family=Shadows+Into+Light&display=swap');
        .loginContainer {
        top:20vh;
        display: flex;
        position: sticky; 
  ...
  `;
    ...
    if (user.entry == 2) {
        ...
        clientLogin(user.clientEmail,user.clientPassword)
        authenticationSucessful(res,1);
        ...
        if (otpVerificationFound != null) { // save in database
            try {
                ...
                createNote(user_name, user_age, user_email, user_password, user_profile, user_other_data);
                authenticationSucessful(res,1);
            } catch (error) {
                console.log(error);
